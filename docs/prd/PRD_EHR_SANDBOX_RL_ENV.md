...
| Rollout driver (JSON-in-text action protocol) | `rollout` at `asclepius/environments/rollout.py:68` | **Unchanged.** The new harness (`ehr_sandbox/harness.py`) adds native tool calling through `call_llm` (`ai/llm_client.py:504`) and imports `_extract_json` for the JSON fallback. |
...
- **Native tool calling:** pass the tool schemas as `tools=` through `call_llm` (`ai/llm_client.py:504`).
  - This works for Anthropic models today.
  - **The OpenAI path drops tools:** `_openai_create_async` (`ai/llm_client.py:304`) sends only flattened text. M4 adds an additive branch there: when `tools` is passed, send them as Responses-API function tools and map `function_call` outputs back to the Anthropic-shaped `tool_use` blocks the rest of the client already returns. Until that lands, OpenAI models run on the JSON-protocol fallback, and the report says which harness each model used.
