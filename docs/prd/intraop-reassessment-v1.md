...
| Primary user | Operating surgeon (fills + locks the form on "Switch to post-op") |
| Secondary users | RN care coordinator (consumes resulting tier in queue), Anesthesia provider (read-only of auto-populated fields), Clinical Operations Lead (audit, reopen rights) |
...
- The surgeon is the sole owner of the form. There is no PACU RN finalizer step and no multi-contributor coordination. AIMS/EHR auto-population happens if upstream data is available, but the surgeon retains all editorial control and is the locker.
...
6. Form lifecycle: NEW → IN_PROGRESS → READY_FOR_LOCK → LOCKED, with admin REOPEN.
...
The pre-op tier captures who walked in and how engaged they were heading into surgery. The post-op signals will tell us how recovery unfolds. Between them is the surgery itself — the single largest perturbation of physiology in the episode and one of the strongest predictors of 30-day complications. A pre-op TIER_1 patient who has a 4-hour case with 800ml EBL, sustained hypotension, and a vasopressor requirement is not the same patient at PACU as they were on the operating table — and the system that drives post-op cadence and threshold sensitivity needs to know that *before* the first home vital arrives 12 hours later. This feature closes that gap. The surgeon, on clicking "Switch to post-op," is presented with the structured intra-op summary, can either upload the operative note for AI extraction or fill the 11 required fields manually, confirms, and locks; the system then re-runs the tier computation with those data points folded in and writes the new tier with full audit.
...
The form opens with two visible affordances:

1. **Upload operative note (PDF)** — a drop zone. On upload, the file is stored to object storage, an `IntraopExtraction` job runs the LLM extractor, and field values + per-field confidence scores stream into the form. Each extracted field shows a confidence pill (`HIGH ≥0.85`, `MED 0.65–0.85`, `LOW <0.65`) and a "Confirm" toggle. Surgeon must explicitly confirm `LOW`-confidence fields before they count toward locking; `MED` and `HIGH` are confirmed by default but show a "review" affordance. Original PDF is preserved; extraction output is preserved separately for audit.

2. **Fill manually** — a "Fill manually instead" link expands the form into a fully editable state. Surgeon types into the 11 required fields directly. Auto-population (if AIMS data is available) populates fields independently of PDF extraction; both paths write into the same form state with origin tracked per field (`MANUAL | AUTO_POP_AIMS | AUTO_POP_PDF`).

The two paths are **not mutually exclusive**: surgeon can upload a PDF, accept some extracted fields, and manually edit others. Origin metadata is recorded per field per write.

### 3.3 Lock behavior

The **Lock & switch to post-op** button is the only way out of the form. It enables only when:

- All 11 required fields (§4.1) have a value AND a confirmed origin.
- A confirmation modal shows the proposed tier change and contributing reasons.
- Surgeon confirms.
...
3. Episode tier is updated if the resolved tier differs from current.
4. Episode `status` transitions to `POST_OP`.
5. Surgeon is redirected to the post-op episode view.
...
- **AC-3.7** A locked form is read-only; only an admin REOPEN can re-edit (§7.3).
...
interface FieldOrigin {
  origin: 'MANUAL' | 'AUTO_POP_AIMS' | 'AUTO_POP_PDF';
...
```
[NEW]                  ← created when episode records OR_ENDED
   │
   │ first user opens it (surgeon clicks Switch to post-op)
   ▼
[IN_PROGRESS]          ← any field touched, autosave on change (debounced 500ms)
   │
   │ all required fields filled and confirmed
   ▼
[READY_FOR_LOCK]       ← lock button enables
   │
   │ surgeon clicks Lock & switch to post-op
   ▼
[LOCKED]               ← reassessment fires; episode → POST_OP
   │
   │ (rare) admin reopens with reason
   ▼
[REOPENED]             ← editable again; new lock fires a NEW reassessment event (audit chain preserved)
```
...
Only an Admin or Clinical Operations Lead can reopen a LOCKED form. Reopen requires a reason (≥30 chars). On reopen:

- `IntraopForm.status = REOPENED`.
- An audit event captures actor, reason, prior locked snapshot.
- Subsequent re-lock fires a new `IntraOpReassessmentEvent`. The episode tier is re-resolved with the new proposed tier; resolution still uses "most conservative wins" against the *current* tier (which already reflects the prior reassessment).
...
- **AC-7.4** Reopen is admin-only and requires reason ≥30 chars.
...
model IntraopForm {
  ...
  status                      IntraopFormStatus  // NEW | IN_PROGRESS | READY_FOR_LOCK | LOCKED | REOPENED
...
enum IntraopFormStatus { NEW IN_PROGRESS READY_FOR_LOCK LOCKED REOPENED }
...
**`PATCH /api/episodes/:episodeId/intraop-form`** — partial update (autosave).
...
**`POST /api/episodes/:episodeId/intraop-form/lock`** — lock + run reassessment + transition to POST_OP.
...
**`POST /api/episodes/:episodeId/intraop-form/reopen`** — admin-only.
