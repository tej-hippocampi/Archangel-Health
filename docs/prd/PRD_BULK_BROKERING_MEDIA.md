...
9. Admin download previously loaded the complete raw object into memory. It now streams encrypted frames through the reader at backend/routers/asclepius.py:6807.
10. Completion queues clinical ingestion in the web process at backend/routers/asclepius_provider.py:1456. `process_upload` at backend/asclepius/ingestion.py:1771 scans, unpacks, and builds clinical cases. There is no dedicated media catalog/transfer worker in this path.
