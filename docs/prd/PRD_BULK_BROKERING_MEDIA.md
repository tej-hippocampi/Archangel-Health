...
3. The regular multipart request is capped at **100 MiB**. `max_zip_bytes` is at backend/asclepius/ingestion.py:52. Selecting more than one file uses that regular path even when the files are large; the `CHUNKED_MIN_BYTES` branch is at frontend/provider/provider.js:728.
4. The browser pre-hashes the entire file before transfer and sends parts sequentially, stopping on the first failed part. The existing flow can resume after reselecting a file, but has no persistent multi-file queue or automatic part retry: frontend/provider/provider.js:654 and frontend/provider/provider.js:689. The picker in frontend/provider/index.html excludes video extensions.
5. A ZIP member has a **64 MiB extraction cap**. `max_entry_bytes` is at backend/asclepius/ingestion.py:81. `_classify` at backend/asclepius/ingestion.py:900 returns unsupported for MP4, MOV, MKV, and AVI. The clinical pipeline rejects a bundle without parseable clinical content through `_fail` at backend/asclepius/ingestion.py:1983. An upload receipt therefore does not establish that video was accepted as a usable asset.
6. Raw storage is local and normally placed beside the database. `_default_ingest_dir` is at backend/asclepius/ingestion.py:124. Upload admission budgets **4× file size** for working copies; `disk_amplification_factor` is at backend/asclepius/uploads.py:142. Concurrent organizations can compete for that same disk; a free-space check is not a global capacity reservation.
...
8. Raw originals are generally eligible for deletion after **30 days**. `raw_retention_days` is at backend/asclepius/ingestion.py:117; `purge_expired_raw` at backend/asclepius/ingestion.py:535 now exempts brokering/storage originals as well as retained parser failures and fails closed if retention metadata cannot be read. Do not depend on a video parse failure to preserve a commercial asset.
...
10. Completion queues clinical ingestion in the web process at backend/routers/asclepius_provider.py:1444. `process_upload` at backend/asclepius/ingestion.py:1804 scans, unpacks, and builds clinical cases. There is no dedicated media catalog/transfer worker in this path.
