...
- Email (`build_community_digest_post_email`, `onboarding_emails.py:2092`): same hierarchy — Top story with deck
...
2. **Design tokens.** Tag tints are new `cm-tag-*` rules in `community.css` using the
   ...
   token.
3. **Founders' Calendly on profile cards.** `FOUNDER_CALENDLY` lives in `asclepius.js:3152`,
