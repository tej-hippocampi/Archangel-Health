...
- So: 13:00 UTC the digest posts and emails everyone → you deploy (you deploy many
  ...
  `ensure_default_channels()` (`community/store.py:711`, `main.py:6983`) re-creates the seven
...
3. **Fail closed in production:** extend the `main.py:6676` production gate to the
...
| `/internal/community/purge` | manual, internal-auth only (`main.py:7293`); deletes all bot posts | keep, but log an audit event with actor; never call from a scheduler |
