...
def test_tenant_login_roundtrip(triage_seeded):
    ...
    r = client.post(
        ...
        json={"email": TRIAGE_DEMO_SURGEON_EMAIL, "password": TRIAGE_DEMO_SURGEON_PASSWORD},
    )
...
def test_sandra_timeline_day18_clean(surgeon_client, triage_seeded):
    pid = _sandra_id()
    ts: TeamStore = app.state.team_store
    episode = ts.get_episode(pid)
    assert episode
    from datetime import date, datetime, timedelta

    open_dt = date.fromisoformat(episode["open_date"])
    day18_at = datetime.combine(open_dt + timedelta(days=17), datetime.min.time()).replace(microsecond=0).isoformat()
    ts.log_event(
        patient_id=pid,
        event_type="POSTOP_RETIER_RECOMPUTED",
        occurred_at=day18_at,
        payload={"note": "test noise"},
    )
    ts.log_event(
        patient_id=pid,
        event_type="POSTOP_DAILY_CHECKIN_SENT",
        occurred_at=day18_at,
        payload={"episodeDay": 18},
    )
    from triage_demo_seed import _maintain_sandra_reyes_timeline

    _maintain_sandra_reyes_timeline(ts)

    r = surgeon_client.get(f"/api/patient/{pid}/timeline")
    assert r.status_code == 200
    day18 = (r.json().get("markers") or {}).get("18") or []
    assert day18 == []
...
def test_triage_preserve_seed_is_idempotent():
    ts: TeamStore = app.state.team_store
    store = app.state.patient_store
    ensure_triage_demo_staff(ts)
    t0 = date(2026, 5, 1)
    ...
    sandra = _sandra_id()
    proc_before = (store[sandra].get("structured_data") or {}).get("procedure_date")
    ...
    seed_triage_demo_sqlite(ts, store, strategy="preserve", today=t1)

    proc_after = (store[sandra].get("structured_data") or {}).get("procedure_date")
...
def test_preop_retier_events_seeded_for_preop_demo():
    ...
    seed_triage_demo_sqlite(ts, store, strategy="reset")
    pid = _michael_id()
    events = ts.list_preop_retier_events(pid, limit=5)
    assert events, "expected seeded pre-op retier snapshot rows"
    assert all(e.get("triggered_by") == "demo:seed-preop-retier" for e in events)
