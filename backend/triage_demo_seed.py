...
def build_patient_blob(
    ...
    *,
    ...
    battlecard_fn: DemoBattlecardFn,
    idx: int,
    open_date_override: Optional[date] = None,
) -> Dict[str, Any]:
    ...
    if phase == "pre_op":
        open_d = open_date_override or today
        proc_d = open_d + timedelta(days=int(row["surgery_offset_days"]))
        ...
        open_d = open_date_override or (today - timedelta(days=max(0, dday - 1)))
    ...
    blob: Dict[str, Any] = {
        ...
        "tier_last_changed": _dt_combine(open_d + timedelta(days=max(0, int(row.get("episode_day", 1)) - 1))),
        "resources": resources,
        "pcp_referral_sent": idx % 3 != 0,
        "pcp_name": None,
    }
...
def merge_triage_patients_into_store(
    ...
    *,
    ...
    for idx, row in enumerate(rows):
        open_override: Optional[date] = None
        if strategy == "preserve" and team_store is not None:
            ep = team_store.get_episode(row["id"]) or {}
            open_s = str(ep.get("open_date") or "").strip()
            if open_s:
                try:
                    open_override = date.fromisoformat(open_s)
                except ValueError:
                    open_override = None
        patient_store[row["id"]] = build_patient_blob(
            ...
            idx=idx,
            open_date_override=open_override,
        )
...
def _seed_sandra_reyes_clinical(team_store: Any, patient_store: Dict[str, Any], open_d: date, today: date) -> None:
    ...
    team_store.save_postop_retier_event(
        ...
        inputs_snapshot={"day": 7, "days_since_discharge": 7},
        post_intraop_tier="TIER_2",
        computed_delta=1,
...
def _maintain_sandra_reyes_timeline(team_store: Any) -> None:
    """Keep Sandra Reyes day 18+ empty — purge cron noise that accumulates over time."""
    pid = "triage_sandra_reyes"
    episode = team_store.get_episode(pid)
    if not episode:
        return
    team_store.delete_event_logs_from_episode_day(pid, min_day=18)
    ...
    *,
    ...
    for idx, row in enumerate(rows):
        ...
        team_store.upsert_episode_snapshot(
            ...
            post_intake_tier=row.get("current_tier") if row.get("preop_retier") else None,
        ...
        if row.get("preop_retier") and row["phase"] == "pre_op" and pid != "triage_patricia_alvarez":
            current_tier = row.get("current_tier", init_tier)
            changed = str(current_tier).upper() != str(init_tier).upper()
            team_store.save_preop_retier_event(
                event_id=uuid.uuid4().hex,
                episode_id=pid,
                triggered_by="demo:seed-preop-retier",
                inputs_snapshot={"seeded": True},
                initial_tier=init_tier,
                initial_tier_was_hard=hard,
                computed_delta=3 if changed else 0,
                computed_tier=current_tier,
    ...
    for i, (pid, etier, trig) in enumerate(esc_spec):
        if pid not in patient_store:
            continue
        if trig == "PATIENT_SELF_FLAG_ACTIVE" and not team_store.has_active_self_flag(pid):
            team_store.create_self_flag(
                patient_id=pid,
                free_text="Seeded demo self-flag for tier-3 escalation.",
                source="DEMO_SEED",
            )
        if team_store.has_open_escalation(pid, trig):
            continue
